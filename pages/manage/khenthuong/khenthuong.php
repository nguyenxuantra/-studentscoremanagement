          ...
          <li class="nav-item menu-items">
            ...
            <div class="collapse" id="auth">
              <ul class="nav flex-column sub-menu">
                ...
                <li class="nav-item"> <a class="nav-link" href="../../samples/register.php"> Đăng ký </a></li>
              </ul>
            </div>
          </li>
              ...
              <li class="nav-item dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="profileDropdown">
                  ...
                  <form action="" method="post">
                    <div class="preview-item-content">
                      <button name="logout" class='btn ' type="submit">Đăng xuất</button>
                    </div>
                  </form>
                  </a>
                  <div class="dropdown-divider"></div>
...
<form action="" method="post">
    ...
    <div class="row">
      ...
      <select class="form-control col-sm-3 mt-4 mx-auto" name="selected_id_lop" aria-label="Default select example">
      
        <option selected>Mã lớp</option>
        <?php $value=1; foreach($statement as $row) {?>
        <option value="<?php echo $value ?>"><?php echo $row['id_lop'] ?></option>
        <?php $value++; } ?>
      </select>
...
require_once "../../../connection/connect.php";
    ...
    if(isset($_POST['btn_search'])) {
        $search = $_POST['search'];
        $id_lop = $_POST['selected_id_lop'];
        // Kiểm tra xem thông tin tìm kiếm có rỗng không
        if($search == "") {
            if($id_lop!="Mã lớp")
            {
              $sql_search = "SELECT * FROM sinhvien WHERE id_lop=$id_lop";
              ...
              if($row_count_search > 0) {
                  ...
                  echo "<thead><tr><th>Id sinh viên</th><th>Id lớp</th><th>Id chuyên ngành</th><th>Id khoa</th><th>Họ tên</th><th>Quê quán</th><th>Ngày sinh</th><th><a class='btn btn-primary' href='insert.php'>Thêm</a></th></tr></thead><tbody>";
                  foreach($statement_search as $row) {
                      echo "<tr>";
                      echo "<th>{$row['id_sv']}</th>";
                      echo "<th>{$row['id_lop']}</th>";
                      echo "<th>{$row['id_cn']}</th>";
                      echo "<th>{$row['id_khoa']}</th>";
                      echo "<th>{$row['ho_ten']}</th>";
                      echo "<th>{$row['que_quan']}</th>";
                      echo "<th>{$row['ngay_sinh']}</th>";
                      echo "<th><a class='btn btn-warning' href='delete.php?id={$row['id_sv']}'>Xóa</a> &nbsp;<a class='btn btn-warning' href='update.php?id={$row['id_sv']}'>Sửa</a></th>";
                      echo "</tr>";
                  }
                  echo "</tbody></table>";
            } else {
                echo "nhập thông tin tìm kiếm";
            }
            }
        } else {
            // Thực hiện truy vấn SQL tìm kiếm
            $sql_search = "SELECT * FROM sinhvien WHERE id_sv LIKE '%$search%' OR ho_ten LIKE '%$search%' OR que_quan LIKE '%$search%'";
            $statement_search = $conn->query($sql_search);
        ...
        $sql_all = "SELECT * FROM sinhvien";
        $statement_all = $conn->query($sql_all);
        echo "Danh sách sinh viên được khen thưởng:";
        echo "<table class='table table-dark tabel-hover'>";
        echo "<thead><tr><th>Id sinh viên</th><th>Id lớp</th><th>Id chuyên ngành</th><th>Id khoa</th><th>Họ tên</th><th>Quê quán</th><th>Ngày sinh</th><th><a class='btn btn-primary' href='insert.php'>Thêm</a></th></tr></thead><tbody>";
        foreach($statement_all as $row) {
            echo "<tr>";
            echo "<th>{$row['id_sv']}</th>";
            echo "<th>{$row['id_lop']}</th>";
            echo "<th>{$row['id_cn']}</th>";
