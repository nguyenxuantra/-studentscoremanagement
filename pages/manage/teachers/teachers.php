...
<html lang="en">
  ...
  <body>
    ...
    <div class="row">
      ...
      <select class="form-control col-sm-3 mt-4 mx-auto" name="selected_id_khoa" aria-label="Default select example">
      
        <option selected>Mã khoa</option>
        <?php $value=1; foreach($statement as $row) {?>
        <option value="<?php echo $value ?>"><?php echo $row['id_khoa'] ?></option>
        <?php $value++; } ?>
      </select>
...
require_once "../../../connection/connect.php";
    ...
    if(isset($_POST['btn_search'])) {
        $search = $_POST['search'];
        $id_khoa = $_POST['selected_id_khoa'];
        // Kiểm tra xem thông tin tìm kiếm có rỗng không
        if($search == "") {
            if($id_khoa!="Mã khoa")
            {
              $sql_search = "SELECT * FROM giangvien WHERE id_khoa=$id_khoa";
              ...
              if($row_count_search > 0) {
                  ...
                  echo "<thead><tr><th>Id giảng viên</th><th>Id khoa</th><th>Họ tên</th><th>Quên quán</th><th>Ngày sinh</th><th><a class='btn btn-primary' href='insert.php'>Thêm</a></th></tr></thead><tbody>";
                  foreach($statement_search as $row) {
                      echo "<tr>";
                      echo "<th>{$row['id_gv']}</th>";
                      echo "<th>{$row['id_khoa']}</th>";
                      echo "<th>{$row['ho_ten']}</th>";
                      echo "<th>{$row['que_quan']}</th>";
                      echo "<th>{$row['ngay_sinh']}</th>";
                      echo "<th><a class='btn btn-warning' href='delete.php?id={$row['id_gv']}'>Xóa</a> &nbsp;<a class='btn btn-warning' href='update.php?id={$row['id_gv']}'>Sửa</a></th>";
                      echo "</tr>";
                  }
                  echo "</tbody></table>";
            } else {
            ...
            $sql_search = "SELECT * FROM giangvien WHERE id_gv LIKE '%$search%' OR ho_ten LIKE '%$search%' OR que_quan LIKE '%$search%'";
            $statement_search = $conn->query($sql_search);
