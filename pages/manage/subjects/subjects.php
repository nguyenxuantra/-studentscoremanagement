...
<?php
        ...
        if (isset($_POST['btn_search'])) {
            ...
            if ($search == "") {
                ...
                $sql_search = "SELECT * FROM monhoc WHERE 
                    id_mh LIKE '%$search%' OR
                    ten_mh LIKE '%$search%' OR
                    so_tin LIKE '%$search%' ";
                ...
                if ($row_count_search > 0) {
                    echo "Tìm thấy kết quả:".'<div class="row d-flex justify-content-end"><a href="subjects.php" class="btn btn-primary mt-1 mb-2 mr-3 ">Trở lại</a></div>';
                    // Hiển thị kết quả tìm kiếm
                    echo "<table class='table table-dark table-hover'>";
                    echo "<thead><tr><th>Id môn học</th><th>Tên môn học</th><th>Số tín</th><th>Action</th></tr></thead><tbody>";
                    foreach ($statement_search as $row) {
                        echo "<tr>";
                        echo "<th>{$row['id_mh']}</th>";
                        echo "<th>{$row['ten_mh']}</th>";
                        echo "<th>{$row['so_tin']}</th>";

                        echo "<th><a class='btn btn-warning' href='delete.php?id={$row['id_mh']}'>Xóa</a> &nbsp;<a class='btn btn-warning' href='update.php?id={$row['id_mh']}'>Sửa</a></th>";
                        echo "</tr>";
                    }
                    echo "</tbody></table>";
                } else {
            ...
            // Hiển thị bảng user khi không có chức năng tìm kiếm được sử dụng
            $sql_all = "SELECT * FROM monhoc";
            $statement_all = $conn->query($sql_all);
            echo "Danh sách user:";
            echo "<table class='table table-dark table-hover'>";
            echo "<thead><tr><th>Id môn học</th><th>Tên môn học</th><th>Số tín</th><th><a class='btn btn-primary' href='insert.php'>thêm</a></th></tr></thead><tbody>";
