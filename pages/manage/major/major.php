...
<?php
        ...
        if (isset($_POST['btn_search'])) {
            ...
            if ($search == "") {
                ...
                $sql_search = "SELECT * FROM chuyennganh WHERE 
                    id_cn LIKE '%$search%' OR
                    ten_cn LIKE '%$search%' OR
                    khoa LIKE '%$search%'";
                ...
                if ($row_count_search > 0) {
                    echo "Tìm thấy kết quả:".'<div class="row d-flex justify-content-end"><a href="accounts.php" class="btn btn-primary mt-1 mb-2 mr-3 ">Trở lại</a></div>';
                    // Hiển thị kết quả tìm kiếm
                    echo "<table class='table table-dark table-hover'>";
                    echo "<thead><tr><th>Id chuyên nganh</th><th>Tên chuyên ngành</th><th>Khoa</th><th>Action</th></tr></thead><tbody>";
                    foreach ($statement_search as $row) {
                        echo "<tr>";
                        echo "<th>{$row['id_cn']}</th>";
                        echo "<th>{$row['ten_cn']}</th>";
                        echo "<th>{$row['khoa']}</th>";
                        echo "<th><a class='btn btn-warning' href='delete.php?id={$row['id_cn']}'>Xóa</a> &nbsp;<a class='btn btn-warning' href='update.php?id={$row['id_cn']}'>Sửa</a></th>";
                        echo "</tr>";
                    }
                    echo "</tbody></table>";
                } else {
            ...
            $sql_all = "SELECT * FROM chuyennganh";
            $statement_search = $conn->query($sql_all);
            echo "Danh sách user:";
            echo "<table class='table table-dark table-hover'>";
            echo "<thead><tr><th>Id chuyên nganh</th><th>Tên chuyên ngành</th><th>Khoa</th><th><a class='btn btn-primary' href='insert.php'>thêm</a></th></tr></thead><tbody>";
                    foreach ($statement_search as $row) {
                        echo "<tr>";
                        echo "<th>{$row['id_cn']}</th>";
                        echo "<th>{$row['ten_cn']}</th>";
